pipeline {
    ...
    parameters {
        booleanParam(name: 'BUILD_CHUNKING', defaultValue: true, description: 'Chunking Service를 수동으로 빌드하고 배포하려면 체크하세요.')
    ...
    environment {
        // Image & Container
        CHUNKING_IMAGE_NAME = "hebees/chunking"
        CHUNKING_CONTAINER  = "hebees-chunking"
    ...
    stages {
        ...
        stage('Update uv.lock') {
            when {
                anyOf {
                    ...
                    expression { params.BUILD_CHUNKING == true }
                }
            }
            steps {
                sh '''
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    ...
                    def tag = "${CHUNKING_IMAGE_NAME}:${env.BUILD_NUMBER}"
                    sh """
                    set -eux
                    docker build -t ${tag} .
                    """
                    env.CHUNKING_BUILD_TAG = tag
                }
            }
        }
        ...
        stage('Deploy Docker Container') {
            ...
            steps {
                withCredentials([file(credentialsId: 'chunking-repo.env', variable: 'CHUNKING_ENV_FILE')]) {
                    sh '''
                    set -eux
                    # 기존 컨테이너 종료/삭제
                    docker stop "$CHUNKING_CONTAINER" || true
                    docker rm "$CHUNKING_CONTAINER" || true

                    # 컨테이너 실행: --env-file로 환경 변수 주입
                    docker run -d \
                        --name "$CHUNKING_CONTAINER" \
                        --restart unless-stopped \
                        --network "$APP_NETWORK_TEST" \
                        --network "$APP_NETWORK_PROD" \
                        --network "$DB_NETWORK" \
                        --env-file "$CHUNKING_ENV_FILE" \
                        "$CHUNKING_BUILD_TAG"
                    '''
                }
            }
                    ...
                    for (int i = 0; i < maxRetries; i++) {
                        def status = sh(script: '''
                            docker run --rm --network "$APP_NETWORK_TEST" curlimages/curl:8.8.0 \
                                -fsS http://$CHUNKING_CONTAINER:8000/health >/dev/null
                        ''', returnStatus: true)
                        if (status == 0) { ok = true; break }
                        sleep 2
                    }
                    if (!ok) { error "Health check failed for ${CHUNKING_CONTAINER}" }
                }
            }
        }
...
def sendMMNotify(boolean success, Map info) {
    def titleLine = success ? "## :jenkins7: Chunking 서비스 빌드 성공 ✅"
                            : "## :angry_jenkins: Chunking 서비스 빌드 실패 ❌"
    def lines = []
