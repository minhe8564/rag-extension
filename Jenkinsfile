pipeline {
    ...
    parameters {
        booleanParam(name: 'BUILD_EMBEDDING', defaultValue: true, description: 'Embedding Service를 수동으로 빌드하고 배포하려면 체크하세요.')
    ...
    environment {
        // Image & Container
        EMBEDDING_IMAGE_NAME = "hebees/embedding"
        EMBEDDING_CONTAINER  = "hebees-embedding"
    ...
    stages {
        ...
        stage('Prepare Docker Networks') {
            when {
                anyOf {
                    ...
                    expression { params.BUILD_EMBEDDING == true }
                }
            }
            steps {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    ...
                    def tag = "${EMBEDDING_IMAGE_NAME}:${env.BUILD_NUMBER}"
                    sh """
                    set -eux
                    docker build -t ${tag} .
                    """
                    env.EMBEDDING_BUILD_TAG = tag
                }
            }
        }
        ...
        stage('Deploy Docker Container') {
            ...
            steps {
                withCredentials([file(credentialsId: 'embedding-repo.env', variable: 'EMBEDDING_ENV_FILE')]) {
                    sh '''
                    set -eux
                    # 기존 컨테이너 종료/삭제
                    docker stop "$EMBEDDING_CONTAINER" || true
                    docker rm "$EMBEDDING_CONTAINER" || true
                    ...
                    for (int i = 0; i < maxRetries; i++) {
                        def status = sh(script: '''
                            docker run --rm --network "$APP_NETWORK_TEST" curlimages/curl:8.8.0 \
                                -fsS http://$EMBEDDING_CONTAINER:8000/health >/dev/null
                        ''', returnStatus: true)
                        if (status == 0) { ok = true; break }
                        sleep 2
                    }
                    if (!ok) { error "Health check failed for ${EMBEDDING_CONTAINER}" }
                }
            }
        }
...
def sendMMNotify(boolean success, Map info) {
    def titleLine = success ? "## :jenkins7: Embedding 서비스 빌드 성공 ✅"
                            : "## :angry_jenkins: Embedding 서비스 빌드 실패 ❌"
    def lines = []
