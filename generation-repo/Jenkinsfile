pipeline {
    ...
    parameters {
        booleanParam(name: 'BUILD_GENERATION', defaultValue: true, description: 'Generation Service를 수동으로 빌드하고 배포하려면 체크하세요.')
    ...
    environment {
        // Image & Container
        GENERATION_IMAGE_NAME = "hebees/generation"
        GENERATION_CONTAINER  = "hebees-generation"
    ...
    stages {
        ...
        stage('Update uv.lock') {
            when {
                anyOf {
                    ...
                    expression { params.BUILD_GENERATION == true }
                }
            }
            steps {
                sh '''
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    ...
                    def tag = "${GENERATION_IMAGE_NAME}:${env.BUILD_NUMBER}"
                    sh """
                    set -eux
                    docker build -t ${tag} .
                    """
                    env.GENERATION_BUILD_TAG = tag
                }
            }
        }
        ...
        stage('Deploy Docker Container') {
            ...
            steps {
                withCredentials([file(credentialsId: 'generation-repo.env', variable: 'GENERATION_ENV_FILE')]) {
                    sh '''
                    set -eux
                    # 기존 컨테이너 종료/삭제
                    docker stop "$GENERATION_CONTAINER" || true
                    docker rm "$GENERATION_CONTAINER" || true
                    ...
                    docker run -d \
                        ...
                        --network "$DB_NETWORK" \
                        --env-file "$GENERATION_ENV_FILE" \
                        "$GENERATION_BUILD_TAG"
                    '''
                }
            }
        }
                    ...
                    for (int i = 0; i < maxRetries; i++) {
                        def status = sh(script: '''
                            docker run --rm --network "$APP_NETWORK_TEST" curlimages/curl:8.8.0 \
                                -fsS http://$GENERATION_CONTAINER:8000/health >/dev/null
                        ''', returnStatus: true)
                        if (status == 0) { ok = true; break }
                        sleep 2
                    }
                    if (!ok) { error "Health check failed for ${GENERATION_CONTAINER}" }
                }
            }
        }
...
def sendMMNotify(boolean success, Map info) {
    def titleLine = success ? "## :jenkins7: Generation 서비스 빌드 성공 ✅"
                            : "## :angry_jenkins: Generation 서비스 빌드 실패 ❌"
    def lines = []
