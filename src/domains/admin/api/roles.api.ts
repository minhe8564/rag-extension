...
export const getRoleById = async (userRoleNo: string) => {
  const { data } = await api.get<ApiEnvelope<Role>>(`/api/v1/user/roles/${userRoleNo}`);
  return data.result;
};

// 역할 생성
export const createRole = async (payload: { name: string; mode: number }) => {
  const { data } = await api.post<ApiEnvelope<Role>>('/api/v1/user/roles', payload);
  return data.result;
};

export const updateRole = async (userRoleNo: string, payload: Partial<Role>) => {
  await api.put<ApiEnvelope<unknown>>(`/api/v1/user/roles/${userRoleNo}`, payload);
};

// 역할 삭제
export const deleteRole = async (userRoleNo: string) => {
  await api.delete(`/api/v1/user/roles/${userRoleNo}`);
};
