...
class JwtTokenChecker:
    def __init__(self, require_bearer_scheme: bool = True) -> None:
        self.require_bearer_scheme = require_bearer_scheme
    ...
    async def __call__(
        ...
        authorization: Optional[str] = Header(
            default=None,
            alias="Authorization",
            description="게이트웨이가 전달하는 JWT Bearer 토큰",
        ...
        if self.require_bearer_scheme:
            parts = authorization.split()

            if len(parts) != 2 or parts[0].lower() != "bearer":
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Bearer 토큰 형식의 Authorization 헤더가 필요합니다.",
                )
        ...
        if not authorization.strip():
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Authorization 헤더가 비어 있습니다.",
            )

        return authorization


def check_jwt_token(require_bearer_scheme: bool = True):
    return JwtTokenChecker(require_bearer_scheme=require_bearer_scheme)