...
_metrics_client: Optional[Redis] = None
...
def get_metrics_redis_client() -> Redis:
    global _metrics_client
    if _metrics_client is None:
        if not getattr(settings, "redis_url", None):
            raise RuntimeError("Redis settings are not configured (redis_url)")
        _metrics_client = Redis.from_url(
            settings.redis_url,
            decode_responses=True,
            db=getattr(settings, "redis_metrics_db", 8),
        )
...
async def close_metrics_redis_client() -> None:
    global _metrics_client
