...
from ..services.query import partial_update_query_template
...
async def partial_update_query_template_endpoint(
    ...
    try:
        updated_query_group = await partial_update_query_template(
            session=session,
            query_no=queryNo,
            name=request.name,
        ...
        response = BaseResponse[QueryTemplateDetailResponse](
            ...
            message="성공",
            isSuccess=True,
            result=QueryTemplateDetailResponse.from_query_group(updated_query_group)
        )
