...
class MonthlySalesRecord(BaseModel):
    ...
    판매금액: Union[str, int, float] = Field(..., description="판매금액")
    할인액: Union[str, int, float] = Field("0", description="할인액")
    결제금액: Union[str, int, float] = Field(..., description="결제금액 (실판매금액)")
    판매_수: Union[str, int, float] = Field(..., description="판매 수", alias="판매 수")
    반품_수: Optional[Union[str, int, float]] = Field(None, description="반품 수", alias="반품 수")
    납부_수: Union[str, int, float] = Field("0", description="납부 수", alias="납부 수")
...
class WeeklyPatternRecord(BaseModel):
    """요일/시간대별 패턴 레코드"""
    W: Union[str, int, float] = Field(..., description="요일 코드 (1=일요일, 2=월요일, ..., 7=토요일)")
    WEEK: str = Field(..., description="요일명 (일, 월, 화, 수, 목, 금, 토)")
    HOUR: Union[str, int, float] = Field(..., description="시간대 (0-23)")
    방문수: Union[str, int, float] = Field(..., description="방문 수")
    판매금액: Union[str, int, float] = Field(..., description="판매 금액")
...
class CustomerDemographicRecord(BaseModel):
    """고객 연령대별 레코드"""
    년월: str = Field(..., description="년월 (YYYY-MM)")
    AGE: Union[str, int, float] = Field(..., description="연령 코드 (-1=미분류, 0=10대미만, 1=10대, 2=20대, ..., 9=90대)")
    첫방문여부: Union[str, int, float] = Field(..., description="첫방문 여부 (0=재방문, 1=첫방문)")
    건수: Union[str, int, float] = Field(..., description="구매 건수")
    판매금액: Union[str, int, float] = Field(..., description="판매 금액")
...
class ProductRecord(BaseModel):
    ...
    판매_수: Union[str, int, float] = Field(..., description="판매 수량", alias="판매 수")
    판매금액합: Union[str, int, float] = Field(..., description="판매 금액 합계")
