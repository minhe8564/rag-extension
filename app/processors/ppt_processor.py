"""
PPT 프로세서
PowerPoint 파일(.pptx, .ppt)을 PDF로 변환 후 Marker로 처리
"""
import logging
import threading
import torch
...
class PPTProcessor(BaseProcessor):
    """
    PowerPoint 파일(.pptx, .ppt)을 Markdown으로 변환하는 프로세서
    ...
    def _ensure_model(self):
        ...
        with self._lock:
            ...
            try:
                logger.info(f"[PPT Marker 모델] 로딩 시작 - Device: {self._device.upper()}, Dtype: {self._dtype}")
                kwargs = {"device": self._device}
                if self._dtype_t is not None:
                    kwargs["dtype"] = self._dtype_t
                self._model_dict = create_model_dict(**kwargs)
                logger.info(f"[PPT Marker 모델] 로드 완료 - Device: {self._device.upper()}, Dtype: {self._dtype}")
            except TypeError:
                logger.info(f"[PPT Marker 모델] 로딩 시작 (dtype 제외) - Device: {self._device.upper()}")
                self._model_dict = create_model_dict(device=self._device)
                logger.info(f"[PPT Marker 모델] 로드 완료 - Device: {self._device.upper()}")
            
            return self._model_dict
    
    def process(self, file_path: str) -> Dict[str, Any]:
        """
        PPT 파일 -> PDF 변환 -> Marker로 Markdown 변환
        """
        ...
        try:
            # 1. PPT 파일을 PDF로 변환
            logger.info(f"PPT 파일을 PDF로 변환 시작: {file_path}")
            ...
            logger.info(f"PPT 파일 처리 완료: {file_path}")
            ...
            logger.error(f"PPT 처리 실패: {file_path}, 오류: {e}", exc_info=True)
            raise
        finally:
            # 임시 PDF 파일 정리
