...
class GenerationMetricsService:
    """Generation 단계 메트릭 수집 서비스"""

    def __init__(self):
        self.redis_client: Optional[Redis] = None
        self.metrics_key: str = "generation:metrics:response_time"
    ...
    async def record_generation_time(self, time_ms: float, strategy: str = None):
        """
        Generation 응답시간 기록
        ...
        try:
            ...
            logger.debug(f"Recorded generation time: {time_ms:.2f}ms (strategy: {strategy})")
        except Exception as e:
            logger.error(f"Failed to record generation time: {str(e)}", exc_info=True)


# 싱글톤 인스턴스
_metrics_service: Optional[GenerationMetricsService] = None


def get_generation_metrics_service() -> GenerationMetricsService:
    """Generation 메트릭 서비스 싱글톤 인스턴스 반환"""
    global _metrics_service
    if _metrics_service is None:
        _metrics_service = GenerationMetricsService()
