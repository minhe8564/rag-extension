...
class SearchMetricsService:
    ...
    async def record_search_time(self, time_ms: float, strategy: str = None):
        """
        Search 응답시간 기록
        ...
        try:
            ...
            logger.debug(f"Recorded search time: {time_ms:.2f}ms (strategy: {strategy})")
        except Exception as e:
            logger.error(f"Failed to record search time: {str(e)}", exc_info=True)


# 싱글톤 인스턴스
_metrics_service: Optional[SearchMetricsService] = None


def get_search_metrics_service() -> SearchMetricsService:
    """Search 메트릭 서비스 싱글톤 인스턴스 반환"""
    global _metrics_service
    if _metrics_service is None:
        _metrics_service = SearchMetricsService()
