...
class EmbeddingMetricsService:
    """Embedding 단계 메트릭 수집 서비스"""

    def __init__(self):
        self.redis_client: Optional[Redis] = None
        self.metrics_key: str = "embedding:metrics:response_time"
    ...
    async def record_embedding_time(self, time_ms: float, strategy: str = None):
        """
        Embedding 응답시간 기록
        ...
        try:
            ...
            logger.debug(f"Recorded embedding time: {time_ms:.2f}ms (strategy: {strategy})")
        except Exception as e:
            logger.error(f"Failed to record embedding time: {str(e)}", exc_info=True)


# 싱글톤 인스턴스
_metrics_service: Optional[EmbeddingMetricsService] = None


def get_embedding_metrics_service() -> EmbeddingMetricsService:
    """Embedding 메트릭 서비스 싱글톤 인스턴스 반환"""
    global _metrics_service
    if _metrics_service is None:
        _metrics_service = EmbeddingMetricsService()
