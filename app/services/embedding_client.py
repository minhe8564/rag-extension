...
class EmbeddingClient:
    ...
    async def get_image_embedding(
        ...
        """
        Runpod의 embedding API를 호출하여 이미지 임베딩 수행
        
        Args:
            runpod_address: Runpod 주소 (예: "https://xxx-8000.proxy.runpod.net")
            image: 임베딩할 이미지 URL 또는 이미지 데이터
            model_name: 모델 이름 (예: "sentence-transformers/clip-ViT-B-32")
            timeout: 요청 타임아웃 (초)
        ...
        try:
            ...
            logger.info(f"[EmbeddingClient] Requesting image embedding from: {url}, model: {model_name}")
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                logger.info(f"[EmbeddingClient] Image embedding retrieved, dimension: {len(embedding)}")
                return embedding
                
        except httpx.HTTPStatusError as e:
            logger.error(f"[EmbeddingClient] Failed to get image embedding: HTTP {e.response.status_code}, {e.response.text}")
            raise
        except Exception as e:
            logger.error(f"[EmbeddingClient] Error getting image embedding: {str(e)}")
            raise
