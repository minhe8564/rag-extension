...
└──────┬──────┘
       │
┌──────▼──────────┐
│ FastAPI Gateway │ (API Gateway, JWT 인증)
└──────┬──────────┘
       │
   ┌───┴───┬──────────────┬──────────────┐
   │       │              │              │
┌──▼──┐ ┌─▼────┐    ┌────▼────┐   ┌────▼────┐
│Backend│ │Ingest│    │  Extract │   │ Chunking│
│(Spring)│ │Service│    │  Service │   │ Service │
└───┬──┘ └──┬───┘    └────┬────┘   └────┬────┘
    │       │              │              │
    │       └──────┬───────┴──────┬───────┘
    │              │              │
    ...
    │
┌───▼────┐  ┌──────┐  ┌──────┐  ┌──────┐
│  MySQL │  │Redis │  │MongoDB│  │Milvus│
└────────┘  └──────┘  └──────┘  └──────┘
```

**RAG 파이프라인 흐름:**
1. **Ingest**: 문서 업로드 및 수집
2. **Extract**: PDF, DOCX, XLSX, TXT, 이미지 추출
3. **Chunking**: 문서를 의미 있는 청크로 분할
4. **Embedding**: 텍스트/이미지를 벡터로 변환
5. **Search**: 벡터 유사도 검색 (Milvus)
6. **Cross-Encoder**: 검색 결과 재정렬
7. **Generation**: LLM을 통한 최종 답변 생성
